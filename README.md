...
远一点,让落点落在下图红框内。同时调整视角(镜头往下压),让海平面(地平线)落在红框的上边缘:识别的搜索范围是画面宽度 25%~75%、高度 38%~80%(红框画到 71% 为止,是为了避开下方的 UI 排除区),
...
- 找到了鱼漂但没找到底座颜色、点击点退化成兜底位置(匹配框中心偏下,约 65% 高度处)→ `debug/fallback_<时间戳>.png`(带绿框+红点标注,图上还写着证据类型和分数)
...
- 颜色门槛什么都没通过时,按证据强弱依次找(日志里 `pick:` 那行会说是哪一类):
  ...
  (`FLOAT_MAX_BASE_BLOB_SHARE`),说明那是整片同色的水或地形,不是底座,会退回兜底位置(匹配框 65% 高度处,底座在羽毛下面,所以比几何中心偏下)。

...
先判断是不是掉线了:截一张图,看是不是**登录界面**(断线、被顶号、被服务器踢、账号被冻结,
最后都是同一个页面,只是弹窗文案不同)。判断依据是页面上固定的东西,不看弹窗文字:
每个页面认它自己的两个固定控件,**两个都找到才算**:经典旧世登录页是左上角的游戏标志加右下角的"退出"按钮,
测试服登录页是底部的暴雪徽标加"退出"按钮。不依赖只有部分客户端才有的标志。角色选择页不认(各客户端按钮文字不同,恢复也用不上)。
(模板在 `var/login_screen_*.png`,逻辑在 `login_screen.py`)。

- **不在登录界面:** 说明没有断线(还连着游戏),回车只会打开聊天框,所以直接停止,回到"按 F10 重新开始"的状态。
- **在登录界面:** 尝试重新进入游戏:按固定的三次回车,每次按完等一会儿(`RECOVERY_WAITS`,依次为 2、10、18 秒:
  关弹窗、登录页到角色选择、进游戏的加载画面),中间不看画面。之后继续钓鱼。
  是否真的回到了游戏,靠后面的检查兜底:恢复后再连续 10 次没抓到,就停止。细节见 `fishing.py` 里 `try_recover_from_disconnect()` 的注释。
  如果掉线时已经停在没有弹窗的登录页,三次回车里最后一次会落在游戏里,打开聊天框。

...
- 开启后 `find_float()` 还会逐步打印以 `[float] ` 开头的过程日志,方便看某次识别为什么错:
  截图和搜索区域大小 → UI 排除框 → 颜色门控(水面基线、阈值、掩码像素数、丢掉的大块)→
  每个模板的灰度分数与带颜色门控的分数 → 最终选哪个匹配(以及是否因门控没找到而改用无门控结果)→
  点击点是怎么定的(底座颜色质心,或退化成匹配框中心及原因)→ 最终坐标。没找到时会说明是哪一步没过。
...
- 颜色门槛什么都没通过时(日志里是 `using the shape match among base-colored windows`),只在含有浮漂底座
  颜色的窗口里选形状分数最高的(`FLOAT_BASE_COLOR_RANGES`),不再在整个搜索区域里只看形状。实测在
  波纹水面上,只看形状会选中浪尖的淡蓝白色反光(81 次回退里 7 次,真浮漂当时在画面别处、又小又暗),
  加上底座颜色后其中 3 次找对了浮漂、4 次因为浮漂太淡还没显示出来而返回"没找到"交给重试,不再点错地方。

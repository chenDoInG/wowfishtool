...
远一点,让落点落在下图红框内。同时调整视角(镜头往下压),让海平面(地平线)落在红框的上边缘:识别只在红框范围内找,
地平线落在红框中间说明镜头仰得太高,红框里混进了天空和山崖,既容易误判,也浪费了水面范围。
...
- 找到了鱼漂但点击点退化成兜底位置 → `debug/fallback_<时间戳>.png`(带绿框+红点标注,图上还写着证据类型和分数)
...
- 开启后 `find_float()` 还会逐步打印以 `[float] ` 开头的过程日志,方便看某次识别为什么错:
  ...
  含底座颜色的窗口、模板一致的窗口)→ 点击点是怎么定的(底座颜色质心,或退化成匹配框中心及原因)→ 最终坐标。没找到时会
...
- 颜色门槛什么都没通过时,按证据强弱依次找(日志里 `pick:` 那行会说是哪一类):
  ...
  (`FLOAT_MAX_BASE_BLOB_SHARE`),说明那是整片同色的水或地形,不是底座,会退回匹配框中心。
